...
var COHORT_LOOKUP_TIMEOUT_MS = 5;
async function lookupCohort(env, accountId) {
  let querier = env.ACCOUNT_COHORT_QUERIER;
  if (!querier || !accountId)
    return null;
  let ac = new AbortController();
  try {
    let rpc = querier.lookupAccountCohort(accountId.toString());
    rpc.catch(() => {
    });
    ...
    }), res = await Promise.race([rpc, timeout]);
    return res.ok ? res.result ?? null : (console.error("cohort lookup failed", res.errors), null);
  } catch (e) {
    return console.error("cohort lookup failed", e), null;
  } finally {
    ac.abort();
  }
}
...
var AssetWorkerOuter = class extends WorkerEntrypoint {
   ...
   */
  async getCohort() {
    return this.resolvedCohort === void 0 && (this.resolvedCohort = await lookupCohort(
      this.env,
